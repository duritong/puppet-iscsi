...
define iscsi::connection(
  ...
  # those default values into the template.
  ...
  exec{
    'restart_iscsi_daemon_before_discovery':
      refreshonly => true,
      before => Exec['discover_iscsi_targets'],
      command => '/bin/ls /dev/iscsi_* || /etc/init.d/iscsi restart; /bin/true';
    'restart_iscsi_daemon_after_discovery':
      refreshonly => true,
      require => Exec['discover_iscsi_targets'],
      command => '/bin/ls /dev/iscsi_* || /etc/init.d/iscsi restart; /bin/truer';
